...
var servicePoint = new function () {
	...
	this.confirmSettings = function (warnUser) {
		...
		setTimeout(function(){
			if(sessvars.state.workProfileName.toLowerCase().includes('reception')){
				$('#patientCheckInTab').css('visibility','visible');
			}
			else{
				$('#patientCheckInTab').css('visibility','hidden');
			}
		},500);
		var branchSel = $("#branchListModal");
		var workstationSel = $("#workstationListModal");
  ...
  var prepareWantedWorkstation = function () {
	...
	var calledAppointment=0;
	var calledEarlyAppointment=0;
	var calledLateAppointment=0;
	var calledWalkin=0;

	this.callNext = function () {
		var calledByVisitId=false;
debugger;
		if(localStorage.getItem("calledAppointment")){
			calledAppointment= +(localStorage.getItem("calledAppointment"));
		}
		if(localStorage.getItem("calledEarlyAppointment")){
			calledEarlyAppointment= +(localStorage.getItem("calledEarlyAppointment"));
		}
		if(localStorage.getItem("calledLateAppointment")){
			calledLateAppointment= +(localStorage.getItem("calledLateAppointment"));
		}
		if(localStorage.getItem("calledWalkin")){
			calledWalkin= +(localStorage.getItem("calledWalkin"));
		}

		if (!!!minTimeBetweenCallsTimer) {
			var params = servicePoint.createParams();
		/**Customization order by token number for appointment**/
		//Setting max ticket number
		var ticket=1000000;
		var visitId=0;
		var appointmentticket=false;
		var profileWorkId;
		var getWorkProfileQueue;
		var getWorkProfileVisits;
		var getAppointmentVisits=[];
		var getAppointmentQueueId=[];
		var getLateAppointmentVisits=[];
		var getLateAppointmentQueueId=[];
		var getEarlyAppointmentVisits=[];
		var getEarlyAppointmentQueueId=[];
		var getWalkinVisits=[];
		var getWalkinQueueId=[];
		//GetCuurent work profile Id
		var profileWorkIdDetails= spService.putCallback("branches/"+params.branchId+"/servicePoints/"+params.servicePointId+"/users/"+params.userName);
		
		if(profileWorkIdDetails!=null){
		profileWorkId= profileWorkIdDetails.workProfileId;
		}
		
		//Getall the queues inside the work profile
		if(profileWorkId!=null){
		getWorkProfileQueue= spService.get("branches/"+params.branchId+"/workProfiles/"+profileWorkId);
		//kisho visits/full is not available in old api
		//getWorkProfileVisits= spService.get("branches/"+params.branchId+"/workProfiles/"+profileWorkId+"/visits/full");
		getWorkProfileVisits= spService.get("branches/"+params.branchId+"/workProfiles/"+profileWorkId+"/visits");
		}
		//rest/servicepoint/branches/1/workProfiles/1/visits
			//Find the queue id with appointment that needs to be sorted
		if(profileWorkId!=null){

				// for(var j=0;j<getWorkProfileQueue.callingRules[0].callingRuleQueues.length;j++){
			
				// 		if(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment") && !getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment Late") && !getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment Early")){
				// 	//	if(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment")){
				// 			getAppointmentQueueId.push(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].origId);
				// 		}
				// 		else if(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment Late")){
				// 			getLateAppointmentQueueId.push(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].origId);
				// 		}
				// 		else if(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].name.includes("Appointment Early")){
				// 			getEarlyAppointmentQueueId.push(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].origId);
				// 		}
				// 		else{
				// 			getWalkinQueueId.push(getWorkProfileQueue.callingRules[0].callingRuleQueues[j].origId);
				// 		}
				// }
				// for(var j=0;j<getWorkProfileVisits.length;j++){
			
					// if(getWorkProfileVisits[j].parameterMap){
						// if(getWorkProfileVisits[j].parameterMap.level){
							// if(getWorkProfileVisits[j].parameterMap.level=="VIP Level 1"){
								// getAppointmentVisits.push(getWorkProfileVisits[j]);
							// }
							// else if(getWorkProfileVisits[j].parameterMap.level=="VIP Level 2"){
								// getLateAppointmentVisits.push(getWorkProfileVisits[j]);
							// }
							// else if(getWorkProfileVisits[j].parameterMap.level=="VIP Level 3"){
								// getEarlyAppointmentVisits.push(getWorkProfileVisits[j]);
							// }
							// else{
								// getWalkinVisits.push(getWorkProfileVisits[j]);
							// }
						// }
						// else{
							// getWalkinVisits.push(getWorkProfileVisits[j]);
				...
				for(var j=0;j<getWorkProfileVisits.length;j++){
				var visitInf_2=spService.get("branches/"+params.branchId+"/visits/"+getWorkProfileVisits[j].visitId);
					if(visitInf_2.parameterMap){
						if(visitInf_2.parameterMap.level){
							if(visitInf_2.parameterMap.level=="VIP Level 1"){
								getAppointmentVisits.push(visitInf_2);
							}
							else if(visitInf_2.parameterMap.level=="VIP Level 2"){
								getLateAppointmentVisits.push(visitInf_2);
							}
							else if(visitInf_2.parameterMap.level=="VIP Level 3"){
								getEarlyAppointmentVisits.push(visitInf_2);
							}
							else{
								getWalkinVisits.push(visitInf_2);
							}
						}
						else{
							getWalkinVisits.push(visitInf_2);
		...
		// if(calledAppointment<2){
		//get all the tickets in the appointment queue
		// if(getAppointmentQueueId.length!=0){
		// 	getAppointmentQueueId.forEach(getAppointmentQueueId_ => {
		// 		var tempgetAppointmentVisits = spService.get("branches/"+params.branchId+"/queues/"+getAppointmentQueueId_+"/visits");
		// 		if(getAppointmentVisits.length>0){
		// 			if(tempgetAppointmentVisits.length>0){
		// 				getAppointmentVisits=getAppointmentVisits.concat(tempgetAppointmentVisits);
		// 			}
		// 		}
		// 		else{
		// 			getAppointmentVisits=tempgetAppointmentVisits;
		// 		}
		// 	});
	...
	// }
	// else if (calledEarlyAppointment<1){
		//get all the tickets in the appointment queue
		// if(getEarlyAppointmentQueueId.length!=0){
		// 	getEarlyAppointmentQueueId.forEach(getAppointmentQueueId_ => {
		// 		var tempgetAppointmentVisits = spService.get("branches/"+params.branchId+"/queues/"+getAppointmentQueueId_+"/visits");
		// 		if(getEarlyAppointmentVisits.length>0){
		// 			if(tempgetAppointmentVisits.length>0){
		// 				getEarlyAppointmentVisits=getEarlyAppointmentVisits.concat(tempgetAppointmentVisits);
		// 			}
		// 		}
		// 		else{
		// 			getEarlyAppointmentVisits=tempgetAppointmentVisits;
		// 		}
		// 	});
	...
	// }
	// else if(calledLateAppointment<1){
		//get all the tickets in the appointment queue
		// if(getLateAppointmentQueueId.length!=0){
		// 	getLateAppointmentQueueId.forEach(getAppointmentQueueId_ => {
		// 		var tempgetAppointmentVisits = spService.get("branches/"+params.branchId+"/queues/"+getAppointmentQueueId_+"/visits");
		// 		if(getLateAppointmentVisits.length>0){
		// 			if(tempgetAppointmentVisits.length>0){
		// 				getLateAppointmentVisits=getLateAppointmentVisits.concat(tempgetAppointmentVisits);
		// 			}
		// 		}
		// 		else{
		// 			getLateAppointmentVisits=tempgetAppointmentVisits;
		// 		}
		// 	});
	...
	// else{
		//get all the tickets in the appointment queue
		// if(getWalkinQueueId.length!=0){
		// 	getWalkinQueueId.forEach(getAppointmentQueueId_ => {
		// 		var tempgetWalkinVisits = spService.get("branches/"+params.branchId+"/queues/"+getAppointmentQueueId_+"/visits");
		// 		if(getWalkinVisits.length>0){
		// 			if(tempgetWalkinVisits.length>0){
		// 				getWalkinVisits=getWalkinVisits.concat(tempgetWalkinVisits);
		// 			}
		// 		}
		// 		else{
		// 			getWalkinVisits=tempgetWalkinVisits;
		// 		}
		// 	});
	...
	if(calledAppointment<2 && getAppointmentVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getAppointmentVisits!=null){
			if(getAppointmentVisits.length>1){		
				for(var i=0;i<getAppointmentVisits.length;i++){
						if(parseInt(getAppointmentVisits[i].ticketId.slice(1))<parseInt(ticket)){
							ticket=getAppointmentVisits[i].ticketId.slice(1);
							visitId=getAppointmentVisits[i].id;
							appointmentticket=true;
						}
				}		
			}else{
				if(getAppointmentVisits.length>0){
				visitId=getAppointmentVisits[0].id;
				}
			}
		}
	}
	else if (calledEarlyAppointment<1 && getEarlyAppointmentVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getEarlyAppointmentVisits!=null){
			if(getEarlyAppointmentVisits.length>1){		
				for(var i=0;i<getEarlyAppointmentVisits.length;i++){
						if(parseInt(getEarlyAppointmentVisits[i].ticketId.slice(1))<parseInt(ticket)){
							ticket=getEarlyAppointmentVisits[i].ticketId.slice(1);
							visitId=getEarlyAppointmentVisits[i].id;
							appointmentticket=true;
						}
				}		
			}else{
				if(getEarlyAppointmentVisits.length>0){
				visitId=getEarlyAppointmentVisits[0].id;
				}
			}
		}
	}
	else if (calledAppointment<4 && getAppointmentVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getAppointmentVisits!=null){
			if(getAppointmentVisits.length>1){		
				for(var i=0;i<getAppointmentVisits.length;i++){
	...
	else if(calledLateAppointment<1 && getLateAppointmentVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getLateAppointmentVisits!=null){
			if(getLateAppointmentVisits.length>1){		
				for(var i=0;i<getLateAppointmentVisits.length;i++){
						if(parseInt(getLateAppointmentVisits[i].ticketId.slice(1))<parseInt(ticket)){
							ticket=getLateAppointmentVisits[i].ticketId.slice(1);
							visitId=getLateAppointmentVisits[i].id;
							appointmentticket=true;
						}
				}		
			}else{
				if(getLateAppointmentVisits.length>0){
				visitId=getLateAppointmentVisits[0].id;
				}
			}
		}

	}
	else if (calledAppointment<6 && getAppointmentVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getAppointmentVisits!=null){
			if(getAppointmentVisits.length>1){		
				for(var i=0;i<getAppointmentVisits.length;i++){
	...
	else if(calledWalkin<1 && getWalkinVisits.length>0){
		//find the minimum token number in the appointment and store it in visitId
		if(getWalkinVisits!=null){
			if(getWalkinVisits.length>1){		
				for(var i=0;i<getWalkinVisits.length;i++){
						if(parseInt(getWalkinVisits[i].ticketId.slice(1))<parseInt(ticket)){
							ticket=getWalkinVisits[i].ticketId.slice(1);
							visitId=getWalkinVisits[i].id;
							appointmentticket=true;
						}
				}		
			}else{
				if(getWalkinVisits.length>0){
				visitId=getWalkinVisits[0].id;
				}
			}
		}		
	}
			...
			if (servicePoint.hasValidSettings()
				...
				debugger;
				if( (getAppointmentVisits==null  && getEarlyAppointmentVisits==null && getLateAppointmentVisits==null && getWalkinVisits==null) || (getAppointmentVisits.length<1 && getEarlyAppointmentVisits.length<1 && getLateAppointmentVisits.length<1 && getWalkinVisits.length<1)){
				...
				// if (callNextResponse) {
					...
					if (sessvars.state.visitState == "CALL_NEXT_TO_QUICK") {
						...
						if (sessvars.state.userState == servicePoint.userState.NO_STARTED_SERVICE_POINT_SESSION) {
							...
							if(getAppointmentVisits!=null  || getEarlyAppointmentVisits!=null || getLateAppointmentVisits!=null || getWalkinVisits!=null){

								if(getAppointmentVisits.length>1 || getEarlyAppointmentVisits.length>1 || getLateAppointmentVisits.length>1 || getWalkinVisits.length>1){
										if(visitId!=0){
										sessvars.state = servicePoint.getState(spService
										.post("branches/" + params.branchId
										+ "/servicePoints/" + params.servicePointId
					...
					if(calledByVisitId){
						if(calledAppointment<2){
							...
							calledEarlyAppointment++;
						}
						else if(calledAppointment<4){
							calledAppointment++;
						} 
						else if(calledLateAppointment<1){
							calledLateAppointment++;
						}
						else if(calledAppointment<6){
							calledAppointment++;
						} 
							...
							calledAppointment=0;
							calledEarlyAppointment=0;
							calledLateAppointment=0;
							calledWalkin=0;
						}
							localStorage.setItem("calledAppointment", ""+calledAppointment);
							localStorage.setItem("calledEarlyAppointment", ""+calledEarlyAppointment);
							localStorage.setItem("calledLateAppointment", ""+calledLateAppointment);
							localStorage.setItem("calledWalkin", ""+calledWalkin);
					}
	...
	var startUserSession = function (params) {
		...
		try {
			...
			else if(newState.servicePointState == servicePoint.servicePointState.CLOSED &&
				newState.branchId == params.branchId ){
					newState.servicePointState="OPEN";
	...
	this.updateWorkstationStatus = function (isRefresh, blockCardChange, blockMesssagePopup,update) {
      ...
      updateNextAndPreviousServices();
		...
		if(sessvars.state.visit){
			$("#nameSpan").html(sessvars.state.visit.parameterMap.custom1);
			$("#hcNOSpan").html(sessvars.state.visit.parameterMap.custom2);
			$("#nationalitySpan").html(sessvars.state.visit.parameterMap.nationality);
			$("#clinicSpan").html(sessvars.state.visit.parameterMap.custom4);
			$("#apptTimeSpan").html(util.formatHHMMSSIntoHHMMA(sessvars.state.visit.parameterMap.custom5));
		}
